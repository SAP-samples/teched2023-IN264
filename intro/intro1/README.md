...
4. <b>Third Subscriber</b> listens to the another AEM queue that is subscribed to the approval topic `SBPA/NewHire/{EmployeeId}/Approval` using the Cloud Integration <b>AMQP</b> sender adapter.
    ...
    <br> 4b. Once the purchase requisition gets created, Cloud Integration send an email to the given newly hired candidate's email with the PR number and direct link to open the same in S/4Hana Cloud system.
